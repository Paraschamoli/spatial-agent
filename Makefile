...
run: env-check ## Run the agent locally
	@echo "🤖 Starting spatial-agent..."
	@uv run python -m spatial_agent

.PHONY: run-dev
run-dev: ## Run the agent in development mode with auto-reload
	@echo "🤖 Starting spatial-agent in development mode..."
	@uv run watchfiles "python -m spatial_agent" spatial_agent/
...
docker-build: ## Build Docker image
	...
	@docker build -f Dockerfile.agent -t spatial_agent:latest .

.PHONY: docker-build-no-cache
docker-build-no-cache: ## Build Docker image without cache
	@echo "🐳 Building Docker image (no cache)..."
	@docker build --no-cache -f Dockerfile.agent -t spatial_agent:latest .
...
docker-clean: docker-rm ## Clean up Docker images and containers
	...
	@docker rmi spatial_agent:latest || true
...
docker-push: ## Push Docker image to Docker Hub
	...
	@docker tag spatial_agent:latest para5/spatial_agent:latest
	@docker push para5/spatial_agent:latest
...
status: ## Show agent and Docker status
	...
	@docker ps -a | grep spatial_agent || echo "No containers found"
	@echo "\nDocker Images:"
	@docker images | grep spatial_agent || echo "No images found"
	@echo "\nAgent Health:"
